...
def render_header(zone_dirs: List[Path]) -> None:
    ...
    st.markdown(
        ...
        <div class="top-hero">
            <div>
                <div class="eyebrow">RETROSPECTIVE MONITORING (T–3 DELAY)</div>
                <h1>GLOBAL MULTISTATION COHERENCE MONITOR</h1>
                <p>
        ...
        <div class="run-strip">
            <b>Key observation:</b> this interface is designed to reveal collective multistation structure that may not be visible from a single station or from amplitude-only inspection.<br><br>
...
def render_quick_guide_outputs() -> None:
    """Render a compact guide explaining how to read the five monitoring outputs."""
    st.markdown("---")
    st.markdown(
        """
### How to read these outputs

The system analyzes short time windows across multiple seismic stations at once.  
...
- **Extreme anomaly distribution**: when the strongest deviations from typical behaviour occur.
- **Mean anomaly and susceptibility**: the global evolution of the network response.
- **Station-resolved z-scores**: how unusual each station is compared with its own baseline.
- **Anomaly vs. synthetic tidal forcing**: comparison with smooth periodic forcing; it is a reference curve, not a prediction signal.

**Important:** these panels must be interpreted together.  
No single graph provides a complete picture of the system.
        """
    )
...
main_tabs = st.tabs(["Monitor", "What is this?", "How it works", "DOI / Papers"])
