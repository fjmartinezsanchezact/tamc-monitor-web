...
try:
    import msvcrt  # Windows timed keyboard input
except ImportError:
    msvcrt = None

try:
    import select  # POSIX fallback
except ImportError:
    select = None
...
def run_multi_monitor() -> int:
    """
    ES: Ejecuta el modo monitor RECENT en varios sitios automáticamente.
    EN: Runs RECENT monitor mode automatically for multiple sites.
    """

    # Fixed monitoring sites.
    # The same RECENT 24 h window and the same T−3 h delay are applied to all regions.
    # Coordinates are reference points used to select the nearest stations automatically.
    sites = [
        # -------------------------
        # Tectonic / seismic layer
        # -------------------------
        {"name": "Torremolinos (Málaga), Spain", "lat": 36.6215, "lon": -4.4994},
                   {"name": "Lorca (Murcia, Spain) — moderate seismic region", "lat": 37.6710, "lon": -1.7010},
	{"name": "Nerpio (Albacete),Spain— low-activity inland region", "lat": 38.150, "lon": -2.300},
        {"name": "Miyako (Japan)", "lat": 39.6414, "lon": 141.9571},
        {"name": "Maule (Chile)", "lat": -35.4264, "lon": -72.3975},
        {"name": "San Andreas Fault (California, USA)", "lat": 35.9000, "lon": -120.4500},
        {"name": "Cascadia Subduction Zone (USA-Canada)", "lat": 45.5000, "lon": -125.0000},
        {"name": "Aegean Sea (Greece-Turkey)", "lat": 38.5000, "lon": 25.0000},
        {"name": "Hikurangi Subduction Zone (New Zealand)", "lat": -39.5000, "lon": 178.5000},
	{"name": "Alaska-Aleutian Subduction Zone (USA)", "lat": 53.5000, "lon": -165.0000},
	{"name": "Kamchatka-Kuril Subduction Zone (Russia-Japan)", "lat": 52.5000, "lon": 160.0000},
	{"name": "Sumatra-Andaman Subduction Zone (Indonesia)", "lat": 2.5000, "lon": 95.5000},
	{"name": "Peru-Ecuador Subduction Zone", "lat": -5.0000, "lon": -80.5000},
	{"name": "North Anatolian Fault (Turkey)", "lat": 40.7000, "lon": 30.5000},
	# Historical / megathrust earthquake reference regions
	{"name": "Tohoku Megathrust (Japan)", "lat": 38.3220, "lon": 142.3690},		
	{"name": "Bengkulu / Sunda Megathrust (Indonesia)", "lat": -4.4380, "lon": 101.3670},
	{"name": "Illapel Subduction Zone (Chile)", "lat": -31.5700, "lon": -71.6700},
	{"name": "Kermadec Subduction Zone (New Zealand)", "lat": -29.0400, "lon": -177.8800},

        # -------------------------
        # Volcanic layer
        # -------------------------
        {"name": "La Palma (Canary Islands)", "lat": 28.6800, "lon": -17.8900},
        {"name": "Reykjanes (Iceland)", "lat": 63.8500, "lon": -22.5600},
        {"name": "Etna (Italy)", "lat": 37.7500, "lon": 14.9900},
        {"name": "Stromboli (Italy)", "lat": 38.7900, "lon": 15.2100},
        {"name": "Kilauea (Hawaii, USA)", "lat": 19.4200, "lon": -155.2900},
        {"name": "Fuego (Guatemala)", "lat": 14.4700, "lon": -90.8800},
        {"name": "Popocatepetl (Mexico)", "lat": 19.0200, "lon": -98.6200},
        {"name": "Yellowstone (Wyoming, USA)", "lat": 44.4280, "lon": -110.5885},
    ]

    print("\n" + "=" * 70)
    print("MULTI MONITOR MODE / MODO MONITOR MULTI-SITIO")
    print("=" * 70)

    rc_max = 0
    for s in sites:
        print("\n" + "=" * 60)
        print(f"[MULTI-MONITOR] {s['name']}")
        print("=" * 60)
        rc = run_recent_monitor(s["name"], s["lat"], s["lon"])
        rc_max = max(rc_max, rc)

    print("\n[OK] Multi monitor completado / completed.")
    return rc_max
...
# Timed menu input
# -------------------------
def timed_menu_input(prompt: str, timeout_seconds: int = 5, default: str = "1") -> str:
    """
    ES:
    - Espera entrada de teclado durante `timeout_seconds`.
    - Si no se pulsa nada, devuelve `default`.
    - En Windows usa msvcrt para no requerir Enter antes del timeout.

    EN:
    - Waits for keyboard input during `timeout_seconds`.
    - If no key is pressed, returns `default`.
    - On Windows uses msvcrt so Enter is not required before timeout.
    """
    ...
    if msvcrt is not None:
        ...
        while True:
            remaining = max(0, timeout_seconds - int(time.time() - start))
            if remaining != last_shown:
                print(
                    f"\r{prompt}Auto-starting option {default} in {remaining:>2}s... ",
                    end="",
                    flush=True,
                )
            ...
            if msvcrt.kbhit():
                ...
                if ch in ("\r", "\n"):
                    print()
                    return buf.strip() if buf.strip() else default
                ...
                if ch.isprintable():
                    buf += ch
                    print(f"\r{prompt}{buf}", end="", flush=True)

            if time.time() - start >= timeout_seconds:
                print()
                print(f"[AUTO] No input detected. Running option {default}.")
                return default
    ...
    if select is not None:
        print(f"(Auto-starting option {default} in {timeout_seconds}s)")
    ...
    # Last resort
    print(f"\n[AUTO] Timed input not available. Running option {default}.")
    return default
...
def main() -> int:
    ...
    print("==============================")
    print("1) Monitor automático multi-sitio / Automatic multi-site monitor")
    print("2) Descargar / Monitorizar manual (Download / Manual monitor)")
    print("3) Procesar data/ existentes (Process existing data/)")
    print("4) RunTests sobre resultados/ (incluye resultados/otros/*)")
    print("5) Análisis inter-eventos (13..18) / Inter-event analysis (13..18)")
    print("6) Clasificación de terremotos (19) / Earthquake clustering (19)")
    print("7) Clasificación por ELI (21+22) / Explosion-likeness index (ELI)")
    print("0) Salir / Exit")

    opt = timed_menu_input("\nOpción / Option:\n> ", timeout_seconds=5, default="1").strip()

    if opt == "1":
        return run_multi_monitor()
    if opt == "2":
